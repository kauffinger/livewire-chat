...
<div class="@if ($isFirst) flex-1 @endif flex w-full flex-row justify-start">
    ...
    >
        ...
        >
            <!-- Tool calls display -->
            <div x-show="hasToolCalls()" class="mb-2 border-b border-zinc-200 pb-2 dark:border-zinc-600">
                <div class="text-sm text-zinc-600 dark:text-zinc-400">Tools used:</div>
                <template
                    x-for="toolCall in streamData.toolCalls"
                    :key="toolCall.name + (toolCall.id || Date.now())"
                >
